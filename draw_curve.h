...
static inline __attribute__ ((always_inline)) void matt_curve8(PERSPECTIVE screen_object, VECTOR3 coordinate_array[], const size_t& len, CRGB& rgb, const bool& flipXY = false, const bool& closedShape = false, const bool& extraSmooth = false, const uint8_t& percentage = 255, const uint8_t& step_size = 32, const bool& additive = false) {
  static const uint8_t val = 255;
  //draw simple lines for step size of 255
  if (step_size == 255) {
    for (int i = 1; i < len; i++) {
      //draw_line_fine2(screen_object, coordinate_array[i-1][0],coordinate_array[i-1][1],coordinate_array[i][0],coordinate_array[i][1],hue,sat,val);
      draw_line_fine(screen_object,coordinate_array[i-1],coordinate_array[i], rgb, -10000, val, val, true,false,true);
    } 
    return;
  }
  ...
  for (int i = starting_point;i<ending_point;i++) {
    ...
    int32_t& x0 = coordinate_array[i0].x;
    int32_t& y0 = coordinate_array[i0].y;
    int32_t& z0 = coordinate_array[i0].z;
       //segment 1 between these points
    int32_t& x1 = coordinate_array[i1].x;
    int32_t& y1 = coordinate_array[i1].y;
    int32_t& z1 = coordinate_array[i1].z;
       //segment 2 between these points
    int32_t& x2 = coordinate_array[i2].x;
    int32_t& y2 = coordinate_array[i2].y;
    int32_t& z2 = coordinate_array[i2].z;
    ...
    while(stp <= 256 && i != -1) {
      ...
      //   return;
      // }
      int32_t percent_done = (stp/total_length) + ((i+1)*256)/total_length;
      ...
      if (stp > 0) {
        ...
        if (flipXY) {
          //draw_line_fine2(screen_object, last_y, last_x, y, x, hue, sat, val);
          VECTOR3 a = VECTOR3(last_y,last_x,last_z);
          VECTOR3 b = VECTOR3(y,x,z);
          draw_line_fine(screen_object, a, b, rgb, -10000, val, val, true,false,true);
            //blendXY(led_screen, y, x, 0, 0, 255);
        } else {
          VECTOR3 a = VECTOR3(last_x,last_y,last_z);
          VECTOR3 b = VECTOR3(x,y,z);
          //draw_line_fine2(screen_object, last_x, last_y, x, y, hue, sat, val);
          draw_line_fine(screen_object, a, b, rgb, -10000, val, val, true,false,false);
            //blendXY(led_screen, x, y, 0, 0, 255);
        }
      }
      //record x,y for the next iteration
      ...
      if (i == ending_point-1 && !closedShape && false) {
        ...
        if (stp > 0) {
          //draw line between points
          if (flipXY) {
            VECTOR3 a = VECTOR3(last_yb,last_xb,last_zb);
            VECTOR3 b = VECTOR3(yb,xb,zb);
            draw_line_fine(screen_object, a, b, rgb, -10000, val, val, true,false,true);
            //blendXY(led_screen, yb, xb, 0, 0, 255);
          } else {
            VECTOR3 a = VECTOR3(last_xb,last_yb,last_zb);
            VECTOR3 b = VECTOR3(xb,yb,zb);
            draw_line_fine(screen_object, a, b, rgb, -10000, val, val, true,false,true);
            //blendXY(led_screen, xb, yb, 0, 0, 255);
          }
        }
