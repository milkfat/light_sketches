...
void generate_html_controls(char *& p) {
...
void generate_sketch_dropdown(char *& p) {
...
void parse_javascript(std::streambuf * inbuf) {
  ...
  while ( inbuf->sgetc() != EOF )
    {
      ...
      //control variables
      if (c >= 48 && c <=57 && inbuf->sbumpc() == '"' && inbuf->sbumpc() == ':' && inbuf->sbumpc() == '"') {
        int cpos = c-48;
